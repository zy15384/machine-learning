...
w1=mu(:, 1)'*wLDA;
sigma1 = sqrt(sum((Sigma(:,:,1)*wLDA).^2));
w2=mu(:, 2)'*wLDA;
sigma2 = sqrt(sum((Sigma(:,:,2)*wLDA).^2));
ratio_s = sigma2/sigma1;
sq_long = sqrt(ratio_s*log(ratio_s));
%x = (w1*sq_long-w2)/(sq_long - 1)
%
z = sym('z');
z = solve((1/sqrt(sigma1))*exp(-(z-w1)^2/(2*sigma1)) - 1/sqrt(sigma2)*exp(-(z-w2)^2/(2*sigma2))==0, z)

% sqrt(sigma2)/sqrt(sigma1)= exp(((x-w2)^2/(x-w1)^2)*(sigma1/sigma2))
% ln(sqrt(sigma2))- ln(sqrt(sigma1)) = ((x-w2)^2/(x-w1)^2)*(sigma1/sigma2)
% (ln(sqrt(sigma2))- ln(sqrt(sigma1)))*(sigma2/sigma1) = ((x-w2)^2/(x-w1)^2)
% sqrt((ln(sqrt(sigma2))- ln(sqrt(sigma1)))*(sigma2/sigma1)) = (x-w2)/(x-w1)
% sqrt((ln(sqrt(sigma2))- ln(sqrt(sigma1)))*(sigma2/sigma1))*(x-w1) = x-w2
% (sqrt((ln(sqrt(sigma2))- ln(sqrt(sigma1)))*(sigma2/sigma1))-1)*x = sqrt((ln(sqrt(sigma2))- ln(sqrt(sigma1)))*(sigma2/sigma1))*w1 - w2
%x = (sqrt((log(sqrt(sigma2))- log(sqrt(sigma1)))*(sigma2/sigma1))*w1 - w2)/(sqrt((log(sqrt(sigma2))- log(sqrt(sigma1)))*(sigma2/sigma1))-1)
%x = fsolve('1/sqrt(1.0054)*exp((x-0)^2/1.0054) - 1/sqrt(1.4901)*exp((x-3.4301)^2/1.4901) = 0','x')
%options = optimset('Display','off');
%x = fsolve(@(x) 1/sqrt(1.0054)*exp((x-0)^2/1.0054) - 1/sqrt(1.4901)*exp((x-3.4301)^2/1.4901), options);
